  ...
  const reports: ReportType[] = [
    {
      ...
      data: () => caseManagerData.map(cm => ({
        ...
        'VL Coverage (%)': (cm.performance.viral_load_results / cm.performance.fy_viral_load_eligible * 100).toFixed(1),
        'VL Coverage Details': `${cm.performance.viral_load_results}/${cm.performance.fy_viral_load_eligible}`,
    ...
    },
    {
      ...
      data: () => cmtData.map(cmt => ({
        ...
        'TX_CURR': cmt.tx_cur,
        'IIT': cmt.iit,
        'Appointment Compliance (%)': cmt.appointments.completion_rate.toFixed(1),
        'Appointment Details': `${cmt.appointments.completed}/${cmt.appointments.scheduled}`,
        'VL Coverage (%)': (cmt.viral_load.results / cmt.viral_load.fy_eligible * 100).toFixed(1),
        'VL Coverage Details': `${cmt.viral_load.results}/${cmt.viral_load.fy_eligible}`,
        'VL Suppression (%)': cmt.viral_load.suppression_rate.toFixed(1),
